...
# settings
mpd_dir = '/junkyard/mp3/'							#mpd music directory
notify_osd = False								#enable/disable notify-osd 
conky = True									#enable/disable conky cover linking
cover_default = '/home/jpoly/.conky/NightDrive/images/cover/cover_bg.png'	#default cover for notify-osd
...
def coverPath():
	...
	# look in album dir
		...
		cvrpath='%s%s'%(mpd_dir,os.path.dirname(last_song))
		print cvrpath
		for img in('cover.jpg', 'folder.jpg', 'front.jpg', 'album.jpg'):
			if cvrpath.endswith("CD1") or cvrpath.endswith("CD2") or cvrpath.endswith("CD3"):
				print cvrpath
				cvrpath=os.path.abspath(os.path.join(cvrpath,".."))
				print cvrpath

			if os.path.exists('%s/%s'%(cvrpath,img)):
				cover= '%s/%s'%(cvrpath,img)
				break
